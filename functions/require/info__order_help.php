...
$q='SELECT * FROM `formetoo_cdb`.`m_contragents` WHERE `m_contragents_id`=1306686034 LIMIT 1;';
$res=$sql->query($q)[0];

$user_actions=$user->getUserActions();
$captcha_feeback=isset($user_actions[17])&&sizeof($user_actions[17])>5?true:false;
if(get('send')=='error')
	...
	<form id="feedback_form" action="/about/feedback/" method="post" novalidate="novalidate">
		<table class="no-border form-table" style="width:60%">
			...
			<tr>
				<td>
					<div class="login_authorization_form_input_container">
						<span class="desc">Мобильный телефон *</span>
					</div>
				</td>
				<td colspan="2">
					<div class="login_authorization_form_input_container">
