...
loadCatalog=function(){
	$.get(
		...
		function(data){
			...
			$(".nav_wrapper > ul").append("<li id=\"nav_more\" class=\"has_sublevel\" ><a href=\"#\"><span>Ещё</span></a><ul class=\"nav_sublevel\">&nbsp;</ul></li>");
			...
			$(window).resize(function(){
				...
				if($(window).width()<1087){//1087
					$(".nav_wrapper > ul > li > a span").css("font-size",".8em");
					$(".nav_account,.nav_cart").addClass("small");
					//$("#menu_id_2000000000").css("width","35%");
				}
				else{
					$(".nav_wrapper > ul > li > a span").css("font-size","1em");
					$(".nav_account,.nav_cart").removeClass("small");
					//$("#menu_id_2000000000").css("width","23%");
				}

				var count_menu=Math.floor($(window).width()/230),
					count_visible_menu=$(".nav_wrapper > ul > li").length-1;
				/* ЕСЛИ КОЛИЧЕСТВО ВИДИМЫХ МЕНЮ МЕНЬШЕ МАКСИМАЛЬНОГО КОЛИЧЕСТВА МЕНЮ, ВОЗВРАЩАЕМ ПУНКТЫ */
				if(count_visible_menu<count_menu){
					for(var i=0;i<count_menu-count_visible_menu;i++){
						$("#nav_more").before($("#nav_more > ul > li:last"));
					}
				}
				else{
					for(var i=0;i<count_visible_menu-count_menu;i++){
						$("#nav_more > ul").append($("#nav_more").prev());
					}
				}
				if($("#nav_more li").length)
					$("#nav_more").show();
				else{
					$("#nav_more").hide();
				}
			...
			$(".nav_wrapper li").each(function(index,el){
				if($(this).children("ul").length)
					$(this).addClass("has_sublevel");
			});

			/* //ТЕНЬ ДЛЯ ПУНКТОВ МЕНЮ КАТАЛОГА ТОВАРОВ
			$(".nav_wrapper > ul > li:first")
				.on("mouseenter",function(){
					var sender=$(this);
						intervalShadow=setTimeout(function(){
							$("#nav_shadow").fadeIn(100);
						},300);
						$("#nav_shadow").dequeue();
				}).on("mouseleave",function(){
						var sender=$(this);
						clearTimeout(intervalShadow);
						$("#nav_shadow").dequeue();
						$("#nav_shadow").fadeOut(100,function(){
							sender.find('ul').hide();
						});
				});
			...
			/* ТЕНЬ ДЛЯ ПУНКТОВ МЕНЮ КАТАЛОГА ТОВАРОВ */
			var intervalShadow,intervalID,intervalID2;
			$(".nav_wrapper > ul > li:first")
				.on("mouseup",function(){
					var sender=$(this);
						intervalShadow=setTimeout(function(){
							...
							intervalID=setTimeout(function(){
								...
								sender.children("ul.nav_sublevel").fadeIn(0,function(){
									...
									$("#menu_id_2000000000 ul.nav_sublevel").css("min-height",menu_height);
									//при открытии главного меню раскрываем без задержки первый пункт
									$('.nav_wrapper > ul > li > ul > li:first').trigger('mouseenter',[1]);
								});
							},0);
						},0);
						$("#nav_shadow").dequeue();
					...
					//если не надо закрыть меню, по умолчанию раскрываем первый пункт
					if(!close&&$('.nav_wrapper > ul > li > ul:visible').length) $('.nav_wrapper > ul > li > ul > li:first').trigger('mouseenter');
					else{
						var sender=$(this);
						clearTimeout(intervalShadow);
						$("#nav_shadow").dequeue();
